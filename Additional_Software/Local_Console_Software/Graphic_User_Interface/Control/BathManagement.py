...
def bathBeginning(beginManager, errorCount): 
    ...
    ser.close()
    beginManager[0] = 1
    return [timerInitialized, timerStatusSet+tempStatusSet+pumpStatusSet+timerStatusStart+tempStatusStart+pumpStatusStartEnd+pumpStatusStart, errorCount]
...
def bathEnding(bathManager, timerInitialized, errorCount):
    ...
    if(tempStatus==0 or pumpStatusEnd==0 or pumpStatus==0):
        ...
        errorCount[0] = 0
    return [isTimerRunning, timerStatus+tempStatus+pumpStatusEnd+pumpStatus, errorCount]
