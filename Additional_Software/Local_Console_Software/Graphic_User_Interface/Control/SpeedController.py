...
def changeFrequency(instrument, frequencyChanged):
    instrument.write_register(100, frequencyChanged, 1)
    ...
    if(isDataRequested):
        ...
        if(len(currentVelocity)>1):
            try:
                ...
                print("There has been an error in Velocity Sensor")
        else:
           dataCount = 0
           isDataRequested = False
...
def checkConnectionMotor():
    ...
    try:
        motorSerial = minimalmodbus.Instrument('/dev/ttyUSB0', 1)
        motorSerial.serial.baudrate = 9600
        motorSerial.write_register(100, 20, 1)
        time.sleep(1)
...
def checkVersionInformation(manager):
    val1= ":01VS01VT"
    res = calculateLRC(val1[1:])
    lrcData = hex((((res^0xFF)+1)&0xFF))
    if(len(lrcData)==3):
    ...
    dataToSend = val1+str(lrcData[2:4]).upper()+'\r\n'
    ser.write(dataToSend.encode('ascii'))
    answerVersionVEL = ser.read(15)
    informationReceived = answerVersionVEL.decode("utf-8")
...
def childProcess(manager, velocityObjective, duration):
    ...
    while(manager[0]!=0):
        ...
        if(manager[0]==13):
            manager = checkVersionInformation(manager)
        ...
        while(manager[1]!=0):
            ...
            speedSensitivity = manager[8]
            motorOrientation = manager[9]
            dataInterval = manager[10]
            manager[16] = 0 #Connection ERROR
            manager[17] = 0 #Sensor ERROR
            ...
            try:
                ...
                ser.open()
                instrument = minimalmodbus.Instrument(motorPortName, 1)
                ...
                while(currentDuration<=objectiveTime and manager[2]==0):
                    ...
                    if(manager[20]==0):
                        ...
                        if(abs(errorCurrent)<controlPrecision): # Default: 1 rpm - BANDA MUERTA
                            errorCurrent = 0
