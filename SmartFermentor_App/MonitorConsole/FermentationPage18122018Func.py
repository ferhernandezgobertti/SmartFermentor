...
class FermentationPage(Frame):
    ...
    def setVariables(self):
        ...
        self.isErrorFound = [False, False, False] # NEEDS TO BE CHECKED WHEN ERROR OCCURS !!!
    ...
    def setMotorParametersManually(self, controller):
        if(self.controlAvailable[0].get()==0):
            if(self.frequencySet.get()==1 and float(self.manualMotorFrequency.get())>=9.0 and float(self.manualMotorFrequency.get())<=50.0):
                ...
                self.velocityInformation[2]['text'] = "Motor manually Set On "+ str(self.manualMotorFrequency.get())+" Hz"
                controller.settingVelocityControl[18] = 1
                controller.settingVelocityControl[19] = int(float(self.manualMotorFrequency.get())*10)
                messagebox.showinfo("Motor Semi Automatic", "Frequency Set at "+self.manualMotorFrequency.get()+" Hz")
            ...
            if(self.frequencySet.get()==0 and float(self.manualMotorVelocity.get())>=180.0 and float(self.manualMotorVelocity.get())<=800.0):
                ...
                self.velocityInformation[2]['text'] = "Motor manually Set On "+ str(self.manualMotorVelocity.get())+" rpm"
                controller.settingVelocityControl[19] = int(float(self.manualMotorVelocity.get())*10)
                messagebox.showinfo("Motor Semi Automatic", "Velocity Set at "+self.manualMotorVelocity.get()+" rpm")
                print("MANUAL MOTOR VELOCITY: ", float(self.manualMotorVelocity.get()))
        else:
            messagebox.showerror("Motor Semi Automatic", "First you must disable the default Velocity Control")
    ...
    def stopMotorManually(self, controller):
        if(self.controlAvailable[0].get()==0):
            ...
            messagebox.showinfo("Motor Semi Automatic", "Motor SUCCESSFULLY Stopped")
        else:
            messagebox.showerror("Motor Semi Automatic", "First you must disable the default Velocity Control")
    ...
    def setBathParametersManually(self, controller):
        if(self.controlAvailable[1].get()==0):
            if(float(self.manualBathTemperature.get())>=10.0 and float(self.manualBathTemperature.get())<=95.0 and int(self.manualBathPumpStep.get())>=1 and int(self.manualBathPumpStep.get())<=5):
                ...
                self.velocityInformation[2]['text'] = "Bath manually Set On "+ str(self.manualBathTemperature.get())+" Celsius (Pump Level "+str(self.manualBathPumpStep.get())+")"
                ...
                messagebox.showinfo("Temperature Semi Automatic", "Bath SUCCESSFULLY Set")
            else:
                messagebox.showwarning("Temperature Semi Automatic", "Please, check Bath Temperature and Pump Level input")
        else:
            messagebox.showerror("Temperature Semi Automatic", "First you must disable the default Temperature Control")

    def stopBathManually(self, controller):
        if(self.controlAvailable[1].get()==0):
            controller.settingTemperatureControl[18] = 3
            messagebox.showinfo("Temperature Semi Automatic", "Bath SUCCESSFULLY Stopped")
        else:
            messagebox.showerror("Temperature Semi Automatic", "First you must disable the default Temperature Control")
    ...
    def expulseAcid(self, controller):
        if(self.controlAvailable[2].get()==0):
            if(int(self.expulsionOption[0].get())>=1 and int(self.expulsionOption[0].get())<=4 and int(self.expulsionOption[1].get())>=1 and int(self.expulsionOption[1].get())<=50):
                ...
                messagebox.showinfo("pH Semi Automatic", "Acid SUCCESSFULLY Expulsed")
            else:
                messagebox.showwarning("pH Semi Automatic", "Please, check Drops and Bursts input")
        else:
            messagebox.showerror("pH Semi Automatic", "First you must disable the default pH Control")
    ...
    def expulseBase(self, controller):
        if(self.controlAvailable[2].get()==0):
            if(int(self.expulsionOption[0].get())>=1 and int(self.expulsionOption[0].get())<=4 and int(self.expulsionOption[1].get())>=1 and int(self.expulsionOption[1].get())<=50):
                ...
                controller.settingPotentialHydrogenControl[17] = int(self.expulsionOption[0].get())
                controller.settingPotentialHydrogenControl[19] = int(self.expulsionOption[1].get())
                messagebox.showinfo("pH Semi Automatic", "Base SUCCESSFULLY Expulsed")
            else:
                messagebox.showwarning("pH Semi Automatic", "Please, check Drops and Bursts input")
        else:
            messagebox.showerror("pH Semi Automatic", "First you must disable the default pH Control")
    ...
    def fillWithVelocityAlternativesOptions(self, controller):
        velocitySemiAutomationBorder = Label(self.canvasSemiAutomation, bg = self.colorORT, height=25, width=70, borderwidth=2, relief='solid', highlightbackground='blue')
        ...
        self.motorFrequencySemiAutomationTitle = Label(self.canvasSemiAutomation, text="MOTOR Frequency", font=self.statusFont, fg = 'blue', bg = self.colorORT)
        self.motorFrequencySemiAutomationTitle.place(x=180, y=100)
        self.motorVelocitySemiAutomationTitle = Label(self.canvasSemiAutomation, text="MOTOR Velocity", font=self.statusFont, fg = 'blue', bg = self.colorORT)
        ...
        self.setAndRunMotor = Button(self.canvasSemiAutomation, text="SET\nMotor\nParameters", command = lambda:self.setMotorParametersManually(controller), relief = SUNKEN, fg='white', bg = 'green4', font=self.buttonFont, compound=CENTER, height = 5, width = 16)
        self.setAndRunMotor.place(x=180, y=250)
        self.stopMotor = Button(self.canvasSemiAutomation, text="STOP\nMotor\nParameters", command = lambda:self.stopMotorManually(controller), relief = SUNKEN, fg='white', bg = 'red', font=self.buttonFont, compound=CENTER, height = 5, width = 16)
        self.stopMotor.place(x=420, y=250)

        self.controlsAvailability.append(Checkbutton(self.canvasSemiAutomation, text="Enable Control", font=self.inputFont, fg='blue', bg=self.colorORT, variable=self.controlAvailable[0], command=lambda:self.changeVelocityControlAvailability(controller)))
        self.controlsAvailability[0].place(x=450, y=20)
        self.setByFrequency = Checkbutton(self.canvasSemiAutomation, text="Frequency Set", font=self.inputFont, fg='blue', bg=self.colorORT, variable=self.frequencySet)
    ...
    def fillWithTemperatureAlternativesOptions(self, controller):
        temperatureSemiAutomationBorder = Label(self.canvasSemiAutomation, bg = self.colorORT, height=11, width=155, borderwidth=2, relief='solid', highlightbackground='orange')
        ...
        self.temperatureBathSemiAutomationTitle = Label(self.canvasSemiAutomation, text="BATH Temperature", font=self.statusFont, fg = 'orange', bg = self.colorORT)
        self.temperatureBathSemiAutomationTitle.place(x=180, y=470)
        self.temperatureBathSemiAutomationInfo = Label(self.canvasSemiAutomation, text="Celsius Grad", font=self.bodyMessageFont, fg = 'orange', bg = self.colorORT)
        self.temperatureBathSemiAutomationInfo.place(x=510, y=470)
        self.setAndRunBathTemperature = Button(self.canvasSemiAutomation, text="SET\n\nBath Circulator\nParameters", command = lambda:self.setBathParametersManually(controller), relief = SUNKEN, fg='white', bg = 'green4', font=self.buttonFont, compound=CENTER, height = 6, width = 18)
        self.setAndRunBathTemperature.place(x=670, y=410)
        self.stopBathTemperature = Button(self.canvasSemiAutomation, text="STOP\n\nBath Circulator\nParameters", command = lambda:self.stopBathManually(controller), relief = SUNKEN, fg='white', bg = 'red', font=self.buttonFont, compound=CENTER, height = 6, width = 18)
        self.stopBathTemperature.place(x=970, y=410)

        self.controlsAvailability.append(Checkbutton(self.canvasSemiAutomation, text="Enable Control", font=self.inputFont, fg='orange', bg=self.colorORT, variable=self.controlAvailable[1], command=lambda:self.changeTemperatureControlAvailability(controller)))
    ...
    def fillWithPotentialHydrogenAlternativesOptions(self, controller):
        potHydrogenSemiAutomationBorder = Label(self.canvasSemiAutomation, bg = self.colorORT, height=25, width=85, borderwidth=2, relief='solid', highlightbackground='green')
        ...
        self.controlsAvailability.append(Checkbutton(self.canvasSemiAutomation, text="Enable Control", font=self.inputFont, fg='green4', bg=self.colorORT, variable=self.controlAvailable[2], command=lambda:self.changePotentialControlAvailability(controller)))
        self.controlsAvailability[2].place(x=1050, y=70)
    ...
    def manageWrongInputFreeWheel(self, controller, optionChosen, isInputCorrect):
        ...
        if(optionChosen>=1 and (velocityInputCorrect or temperatureInputCorrect or potentialHydrogenInputCorrect)):
            messagebox.showinfo("FREE WHEEL", "Selected Magnitudes SUCCESSFULLY Set")
    ...
    def fillWithScrambledInformation(self, pageEntertainment, controller):
        ...
        scrambledGameInfo = Label(pageEntertainment, text=scrambledInformation, font=self.infoFont, fg = 'DodgerBlue3', bg = self.colorORT, justify=LEFT, bd=2, relief='groove', highlightbackground='white')
        scrambledGameInfo.place(x=280, y=55)
    ...
    def fillWithHangmanInformation(self, pageEntertainment, controller):
        ...
        hangmanGameInfo = Label(pageEntertainment, text=hangmanInformation, font=self.infoFont, fg = 'lime green', bg = self.colorORT, justify=LEFT, bd=2, relief='groove', highlightbackground='white')
        hangmanGameInfo.place(x=280, y=335)
    ...
    def fillWithTriviaInformation(self, pageEntertainment, controller):
        ...
        triviaGameInfo = Label(pageEntertainment, text=triviaInformation, font=self.infoFont, fg = 'indian red', bg = self.colorORT, justify=LEFT, bd=2, relief='groove', highlightbackground='white')
        triviaGameInfo.place(x=855, y=335)
