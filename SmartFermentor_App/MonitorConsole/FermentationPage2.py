...
class FermentationPage(Frame):
    ...
    def fillPicturesOnPageSystem(self, pageSystem, controller):
        #controller.setImagesandSeparators(pageSystem, 'systemElement', [115,270,1250,20,115,270,1250,300])
        ...
        #controller.setImagesandSeparators(pageSystem, 'smartElement', [115,240,10,50,115,240,10,300])
    ...
    def runAllControl(self, controller):
        ...
        if(optionChosen==0):
            ...
            if(len(controller.application.listFermentations.fermentations[controller.application.systemCurrentStatus.fermentationActual].magnitudesToControl.potentials)>0):
    ...
    def extractSample(self, controller):
        print("EXTRACTION")
    ...
    def fillPageSystem(self, pageSystem, controller):
        ...
        #self.fillPageSystemEntries(pageSystem, controller)
    ...
    def fillPageFreeWheelLabels(self, pageFreeWheel, controller):
        ...
        self.extractionBorder = Label(pageFreeWheel, bg = self.colorORT, height=7, width=75, borderwidth=5, relief="groove", highlightbackground='DarkGoldenrod3')
        self.extractionBorder.place(x=720, y=300)
        ...
        self.extractionFreeWheelTitle = Label(pageFreeWheel, text=controller.currentLanguage.fermentationPageContent[18], font=self.titleFont, fg = 'DarkGoldenrod3', bg = self.colorORT)
        self.extractionFreeWheelTitle.place(x=745, y=315)
    ...
    def fillPageFreeWheelEntries(self, pageFreeWheel, controller):
        ...
        self.freeWheelInformation.append(Spinbox(pageFreeWheel, from_=5, to=200, font=self.inputFont, fg = self.colorORT, width=10, justify='center')) # extraction
        self.freeWheelInformation[5].place(x=740, y=365)
        self.freeWheelInformation.append(Spinbox(pageFreeWheel, values=("mL", "cL", "dL", "L"), font=self.inputFont, fg = self.colorORT, width=5, justify='center')) # extraction
        self.freeWheelInformation[6].place(x=885, y=365)

    def fillPageFreeWheelButtons(self, pageFreeWheel, controller):
        self.extractionOption = Button(pageFreeWheel, relief = SUNKEN, text=controller.currentLanguage.fermentationPageContent[41], font=self.buttonFont, fg='white', bg='DarkGoldenrod3', height=2, width=20, command = lambda:self.extractSample(controller))
        self.extractionOption.place(x=995, y=330) # STOP ALL
    ...
    def fillPageFreeWheel(self, pageFreeWheel, controller):
        ...
        #self.systemMagnitudesCanvas = FigureCanvasTkAgg(self.systemMagnitudesFigure, pageFreeWheel)
    ...
    def updateVelocityGraphic(self, controller):
        ...
        if(velocityFile.exists()):
            ...
            for eachLine in dataList:
                if len(eachLine)>1:
                    try:   
                        vel, temp, freq, sec, mili = eachLine.split(',')
        ...
        #self.after(10000,lambda:self.updateVelocityGraphic(pageVelocity))
    ...
    def updateTemperatureGraphic(self, controller):
        ...
        if(temperatureFile.exists()):
            ...
            for eachLine in dataList:
                if len(eachLine)>1:
                    try:
                        tempFermentor, tempFermObj, tempBath, tempBathObj, sec, mili = eachLine.split(',')
        ...
        #self.after(10000,lambda:self.updateTemperatureGraphic(pageTemperature))
        ...
        if(controller.application.listFermentations.fermentations[controller.application.systemCurrentStatus.fermentationActual].isTemperatureControlOnFreeWheel):
            ...
            if(len(temperatures)>0):
                ...
                for eachTemperature in temperatures:
                    ...
                    if(position==1):
                        evolutionInformation = controller.currentLanguage.fermentationPageContent[92]
                    elif(temperatures[position-2].valueObjective<eachTemperature.valueObjective):
        ...
        if(potentialFile.exists()):
            ...
            for eachLine in dataList:
                if len(eachLine)>1:
                    ...
                    self.acidDropsList.append(int(acidDrops)-1)
                    self.baseDropsList.append(int(baseDrops)-1)
