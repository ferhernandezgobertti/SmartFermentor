...
class ProfessorPage(Frame):
    ...
    def setVariables(self):
        ...
        self.selfReservation = IntVar()
        self.notificationNow = IntVar()
        self.notificationNear = IntVar()
    ...
    def setLists(self):
        ...
        self.isReservationRight = [True, True, True, True]
    ...
    def placeStaticPictures(self, controller):
        controller.setHeaderSmart(self, 'SmartProfessor', self.logOutImage)
        self.professorInformation = Label(self, text=controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].showUserRegistry(), font=self.infoFont, fg = 'white', bg = self.colorORT)
        self.professorInformation.place(x=845, y=120)
    ...
    def setHelpBar(self, controller):
        ...
        self.canvas = Canvas(self, background="white", width= 890+self.extensionX, height= 320+self.extensionY, highlightthickness=5, highlightbackground=self.colorORT)
        self.canvasConfiguration(controller)

    def configureCanvas(self, controller):
        self.helpBarTitle = Label(self.canvas, text=controller.currentLanguage.userPageContent[0]+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].surname, font=self.subtitleFont, fg=self.colorORT, bg='white')
        self.helpBarTitle.place(x=20,y=20)
        self.helpBarBody = Label(self.canvas, text=controller.currentLanguage.userPageContent[1], font=self.groupMessageFont, fg=self.colorORT, bg='white', justify=LEFT)
    ...
    def refreshTabTitleInformation(self, controller):
        ...
        self.nb.tab(self.pageNewReservation, text = controller.currentLanguage.userPageContent[6])
    ...
    def refreshPageReservationContent(self, controller):
        ...
        self.dateBeginningTitle['text'] = controller.currentLanguage.userPageContent[53]
        self.dateEndingTitle['text'] = controller.currentLanguage.userPageContent[54]
        self.precautionTitle['text'] = controller.currentLanguage.userPageContent[55]

    def refreshPageNewReservationContent(self, controller):
        self.notificationNowOption['text'] = controller.currentLanguage.userPageContent[56]
        self.notificationNearOption['text'] = controller.currentLanguage.userPageContent[57]
        self.nameOfDayBeginning['text'] = controller.currentLanguage.userPageContent[58]+" Monday )"
        self.nameOfDayEnding['text'] = controller.currentLanguage.userPageContent[58]+" Friday )"
        self.selfReservationOption['text'] = controller.currentLanguage.userPageContent[59]
        self.addReservationOption['text'] = controller.currentLanguage.userPageContent[75]
    ...
    def refreshPageContactContent(self, controller):
        ...
        self.sendStudentsNotesOption['text'] = controller.currentLanguage.userPageContent[120]

    def refreshPageProfileContent(self, controller):
        self.fermentationsQuantityTitle['text'] = controller.currentLanguage.userPageContent[96]+" "+str(controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].fermentationsQuantity)
    ...
    def refreshTextContent(self, controller):
        ...
        self.helpBarTitle['text'] = controller.currentLanguage.userPageContent[0]+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].surname
        self.helpBarBody['text'] = controller.currentLanguage.userPageContent[1]
        self.professorInformation['text'] = controller.currentLanguage.userPageContent[2] + controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].registrationDate + " :::: "+ controller.currentLanguage.userPageContent[3] + controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].lastEntryDate
        ...
        self.refreshPageNewReservationContent(controller)
    ...
    def changeLanguage(self, language, controller):
        ...
        if(language=='PORTUGUESE'):
            ...
            self.professorInformation.place(x=860, y=120)
            self.minimizeSystem.place(x=1155, y=470+self.extensionY)
        if(language=='SPANISH'):
            controller.currentLanguage = SpanishLanguage()
            self.professorInformation.place(x=835, y=120)
            self.minimizeSystem.place(x=1155, y=470+self.extensionY)
        if(language=='ENGLISH'):
    ...
    def beginFermentationAction(self, controller):
        currentUser = controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged]
    ...
    def setPicturesOnPagePorts(self, pagePorts, controller):
        controller.setImagesandSeparators(pagePorts, 'portsElement', [115,250,10,10,115,250,10,300])
        controller.setImagesandSeparators(pagePorts, 'portsElement', [115,250,1250,10,115,250,1250,300])
        reservationsSeparator = Picture(['editionProfessorSeparator','png',150,560,640,0],0)
        reservationsSeparator.purpose = 'Separators'
        reservationsSeparatorPic = reservationsSeparator.generateLabel(pagePorts)
        reservationsSeparatorPic.place(x=reservationsSeparator.location[0],y=reservationsSeparator.location[1])
        self.setVelocityLogos(pagePorts, [300, 5, 950, 5])
        self.setTemperatureLogos(pagePorts, [325, 160, 305, 195, 385, 195, 950, 160])
        self.setPotHydrogenLogo(pagePorts, [270, 330, 400, 330])
        screensLogo = Picture(['screensLogo','png',450,100,780,350],0)
        screensLogo.purpose = 'Logos'
        screensLogoPic = screensLogo.generateLabel(pagePorts)
        screensLogoPic.place(x=screensLogo.location[0],y=screensLogo.location[1])

    def fillPagePortsTitles(self, pagePorts, controller):
        self.variatorPortTitle = Label(pagePorts, text=controller.currentLanguage.userPageContent[154], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.variatorPortTitle.place(x=805, y=115)
        self.bathCirculatorPortTitle = Label(pagePorts, text=controller.currentLanguage.userPageContent[155], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.bathCirculatorPortTitle.place(x=805, y=285)
        self.screensPortTitle = Label(pagePorts, text=controller.currentLanguage.userPageContent[156], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.screensPortTitle.place(x=805, y=455)

        self.velocitySensorPortTitle = Label(pagePorts, text=controller.currentLanguage.userPageContent[157], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.velocitySensorPortTitle.place(x=155, y=115)
        self.temperatureSensorPortTitle = Label(pagePorts, text=controller.currentLanguage.userPageContent[158], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.temperatureSensorPortTitle.place(x=155, y=285)
        self.potentialPortTitle = Label(pagePorts, text=controller.currentLanguage.userPageContent[159], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.potentialPortTitle.place(x=155, y=455)

    def fillPagePortsEntries(self, pagePorts, controller):
        self.equipmentPorts.append(Spinbox(pagePorts, values=["/dev/ttyACM0", "/dev/ttyACM1", "/dev/ttyACM2", "/dev/ttyACM3", "/dev/ttyUSB0", "/dev/ttyUSB1", "/dev/ttyUSB2", "/dev/ttyUSB3", "/dev/ttyAMA0", "/dev/ttyAMA1"], font=self.inputFont, width=12, justify='center'))
        self.equipmentPorts[0].place(x=400, y=120)
        self.equipmentPorts.append(Spinbox(pagePorts, values=["/dev/ttyACM1", "/dev/ttyACM2", "/dev/ttyACM3", "/dev/ttyUSB0", "/dev/ttyUSB1", "/dev/ttyUSB2", "/dev/ttyUSB3", "/dev/ttyAMA0", "/dev/ttyAMA1", "/dev/ttyACM0"], font=self.inputFont, width=12, justify='center'))
        self.equipmentPorts[1].place(x=460, y=290)
        self.equipmentPorts.append(Spinbox(pagePorts, values=["/dev/ttyACM2", "/dev/ttyACM3", "/dev/ttyUSB0", "/dev/ttyUSB1", "/dev/ttyUSB2", "/dev/ttyUSB3", "/dev/ttyAMA0", "/dev/ttyAMA1", "/dev/ttyACM0", "/dev/ttyACM1"], font=self.inputFont, width=12, justify='center'))
        self.equipmentPorts[2].place(x=520, y=460)
        self.equipmentPorts.append(Spinbox(pagePorts, values=["/dev/ttyUSB0", "/dev/ttyUSB1", "/dev/ttyUSB2", "/dev/ttyUSB3", "/dev/ttyAMA0", "/dev/ttyAMA1", "/dev/ttyACM0", "/dev/ttyACM1", "/dev/ttyACM2", "/dev/ttyACM3"], font=self.inputFont, width=12, justify='center'))
        self.equipmentPorts[3].place(x=1050, y=120)
        self.equipmentPorts.append(Spinbox(pagePorts, values=["/dev/ttyUSB1", "/dev/ttyUSB2", "/dev/ttyUSB3", "/dev/ttyAMA0", "/dev/ttyAMA1", "/dev/ttyACM0", "/dev/ttyACM1", "/dev/ttyACM2", "/dev/ttyACM3", "/dev/ttyUSB0"], font=self.inputFont, width=12, justify='center'))
        self.equipmentPorts[4].place(x=1050, y=290)
        self.equipmentPorts.append(Spinbox(pagePorts, values=["/dev/ttyUSB2", "/dev/ttyUSB3", "/dev/ttyAMA0", "/dev/ttyAMA1", "/dev/ttyACM0", "/dev/ttyACM1", "/dev/ttyACM2", "/dev/ttyACM3", "/dev/ttyUSB0", "/dev/ttyUSB1"], font=self.inputFont, width=12, justify='center'))
        self.equipmentPorts[5].place(x=1050, y=460)

    def areAllPortsDifferent(self, selectedPort):
        currentPort = 0
        portsDifferent = True
        for eachPort in self.equipmentPorts:
            if(currentPort!=selectedPort and eachPort.get()==self.equipmentPorts[selectedPort].get()):
                portsDifferent = False
                break
            currentPort = currentPort + 1
        return portsDifferent

    def doesPortExist(self, selectedPort):
        portExists = False
        for eachPort in self.portsPossibilities:
            if(eachPort==self.equipmentPorts[selectedPort].get()):
                portExists = True
                break
        return portExists

    def isPortCorrect(self, selectedPort):
        return self.areAllPortsDifferent(selectedPort) and self.doesPortExist(selectedPort)

    def updateProcessPorts(self, portIndicator, selectedPort):
        portCount = 0
        controller.application.systemCurrentStatus.currentControlPorts[selectedPort] = self.equipmentPorts[selectedPort].get()
        for eachPort in self.portsPossibilities:
            if(self.equipmentPorts[selectedPort].get()==eachPort):
                portIndicator = portCount + 1
            portCount = portCount + 1

    def changeVelocitySensorPort(self, controller):
        if(self.isPortCorrect(0)):
            self.updateProcessPorts(controller.settingVelocityControl[21], 0)
            messagebox.showinfo("INFO", controller.currentLanguage.userPageContent[157]+controller.currentLanguage.userPageContent[161])
        else:
            messagebox.showerror("ERROR", controller.currentLanguage.userPageContent[157]+controller.currentLanguage.userPageContent[162])

    def changeTemperatureSensorPort(self, controller):
        if(self.isPortCorrect(1)):
            self.updateProcessPorts(controller.settingTemperatureControl[21], 1)
            messagebox.showinfo("INFO", controller.currentLanguage.userPageContent[158]+controller.currentLanguage.userPageContent[161])
        else:
            messagebox.showerror("ERROR", controller.currentLanguage.userPageContent[158]+controller.currentLanguage.userPageContent[162])

    def changePotentialHydrogenPort(self, controller):
        if(self.isPortCorrect(2)):
            self.updateProcessPorts(controller.settingPotentialHydrogenControl[21], 2)
            messagebox.showinfo("INFO", controller.currentLanguage.userPageContent[159]+controller.currentLanguage.userPageContent[161])
        else:
            messagebox.showerror("ERROR", controller.currentLanguage.userPageContent[159]+controller.currentLanguage.userPageContent[162])

    def changeVelocityVariatorPort(self, controller):
        if(self.isPortCorrect(3)):
            self.updateProcessPorts(controller.settingVelocityControl[22], 3)
            messagebox.showinfo("INFO", controller.currentLanguage.userPageContent[154]+controller.currentLanguage.userPageContent[161])
        else:
            messagebox.showerror("ERROR", controller.currentLanguage.userPageContent[154]+controller.currentLanguage.userPageContent[162])

    def changeTemperatureBathPort(self, controller):
        if(self.isPortCorrect(4)):
            self.updateProcessPorts(controller.settingTemperatureControl[22], 4)
            messagebox.showinfo("INFO", controller.currentLanguage.userPageContent[155]+controller.currentLanguage.userPageContent[161])
        else:
            messagebox.showerror("ERROR", controller.currentLanguage.userPageContent[155]+controller.currentLanguage.userPageContent[162])

    def changeScreensDisplayPort(self, controller):
        if(self.isPortCorrect(5)):
            self.updateProcessPorts(controller.settingScreensControl[22], 5)
            messagebox.showinfo("INFO", controller.currentLanguage.userPageContent[156]+controller.currentLanguage.userPageContent[161])
        else:
            messagebox.showerror("ERROR", controller.currentLanguage.userPageContent[156]+controller.currentLanguage.userPageContent[162])

    def fillPagePortsButtons(self, pagePorts, controller):
        self.changeVelocitySensorPortOption = Button(pagePorts, text=controller.currentLanguage.userPageContent[163], font=self.buttonFont, fg='white', bg='black', relief = SUNKEN, compound=CENTER, command=lambda:self.changeVelocitySensorPort(controller))
        self.changeVelocitySensorPortOption.place(x=455, y=150)
        self.changeTemperatureSensorPortOption = Button(pagePorts, text=controller.currentLanguage.userPageContent[163], font=self.buttonFont, fg='white', bg='black', relief = SUNKEN, compound=CENTER, command=lambda:self.changeTemperatureSensorPort(controller))
        self.changeTemperatureSensorPortOption.place(x=515, y=320)
        self.changePotentialPortOption = Button(pagePorts, text=controller.currentLanguage.userPageContent[163], font=self.buttonFont, fg='white', bg='black', relief = SUNKEN, compound=CENTER, command=lambda:self.changePotentialHydrogenPort(controller))
        self.changePotentialPortOption.place(x=575, y=490)
        self.changeVariatorPortOption = Button(pagePorts, text=controller.currentLanguage.userPageContent[163], font=self.buttonFont, fg='white', bg='black', relief = SUNKEN, compound=CENTER, command=lambda:self.changeVelocityVariatorPort(controller))
        self.changeVariatorPortOption.place(x=1105, y=150)
        self.changeBathPortOption = Button(pagePorts, text=controller.currentLanguage.userPageContent[163], font=self.buttonFont, fg='white', bg='black', relief = SUNKEN, compound=CENTER, command=lambda:self.changeTemperatureBathPort(controller))
        self.changeBathPortOption.place(x=1105, y=320)
        self.changeScreensPortOption = Button(pagePorts, text=controller.currentLanguage.userPageContent[163], font=self.buttonFont, fg='white', bg='black', relief = SUNKEN, compound=CENTER, command=lambda:self.changeScreensDisplayPort(controller))
        self.changeScreensPortOption.place(x=1105, y=490)

    def fillPagePorts(self, pagePorts, controller):
        self.setPicturesOnPagePorts(pagePorts, controller)
        self.fillPagePortsTitles(pagePorts, controller)
        self.fillPagePortsEntries(pagePorts, controller)
        self.fillPagePortsButtons(pagePorts, controller)
    ...
    def updateReservationDisplay(self, pageReserves, controller):
        self.monthNameTitle = Label(pageReserves, text=controller.currentLanguage.userPageContent[28][self.reservationMonthShown], font=self.titleFont, justify='center', fg='white', bg=self.colorORT, height=2, width=15)
    ...
    def setReservesOfMonth(self, pageReserves, controller):
        ...
        self.monthNameTitle.place(x=245, y=0)
        ...
        for week in range(1,6,1):
            ...
            numberOfWeek.place(x=140+2+90*(week-1), y=60)
    ...
    def fillPageReservesButtons(self, pageReserves, controller):
        ...
        previousMonthOption.place(x=790, y=435)
        ...
        nextMonthOption.place(x=1100, y=435)
    ...
    def fillPageReservesEntries(self, pageReserves, controller):
        ...
        self.reservationCheck[4].place(x=770, y=305)
    ...
    def fillPageReservesIndicators(self, pageReserves):
        ...
        reservationYearCheckIndicator = Label(pageReserves, text="YYYY", font="Times 14 bold", fg = 'white', bg = self.colorORT)
        reservationYearCheckIndicator.place(x=1100, y=50)
    ...
    def fillPageNewReservationTitles(self, pageNewReservation, controller):
        self.dateBeginningTitle = Label(pageNewReservation, text=controller.currentLanguage.userPageContent[53], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.dateBeginningTitle.place(x=800, y=20)
        self.dateEndingTitle = Label(pageNewReservation, text=controller.currentLanguage.userPageContent[54], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.dateEndingTitle.place(x=800, y=110)
        self.precautionTitle = Label(pageNewReservation, text=controller.currentLanguage.userPageContent[55], font=self.titleFont, fg = 'white', bg = self.colorORT)
        self.precautionTitle.place(x=800, y=200)
        self.notificationNowOption = Checkbutton(pageNewReservation, text=controller.currentLanguage.userPageContent[56], font=self.inputFont, fg='dark grey', bg=self.colorORT, variable=self.notificationNow)
        self.notificationNowOption.place(x=800, y=410)
        self.notificationNearOption = Checkbutton(pageNewReservation, text=controller.currentLanguage.userPageContent[57], font=self.inputFont, fg='dark grey', bg=self.colorORT, variable=self.notificationNear)
        self.notificationNearOption.place(x=1020, y=400)

        self.nameOfDayBeginning = Label(pageNewReservation, text=controller.currentLanguage.userPageContent[58]+" Monday )", font=self.inputFont, fg = 'white', bg = self.colorORT) # HAY QUE IMPLEMENTAR
        self.nameOfDayBeginning.place(x=820, y=60)
        self.nameOfDayEnding = Label(pageNewReservation, text=controller.currentLanguage.userPageContent[58]+" Friday )", font=self.inputFont, fg = 'white', bg = self.colorORT)
        self.nameOfDayEnding.place(x=820, y=150)

    def fillPageNewReservationPictures(self, pageNewReservation, controller):
        controller.setImagesandSeparators(pageNewReservation, 'studentsElement', [115,250,10,30,115,250,10,290])
        newElement = Picture(['newElement','png',110,170,1230,10],180)
        newElement.purpose = 'Words'
        newElementPic = newElement.generateLabel(pageNewReservation)
        newElementPic.place(x=newElement.location[0],y=newElement.location[1])
        reservationsSeparator = Picture(['reservationsSeparator','png',150,560,640,0],0)
        reservationsSeparator.purpose = 'Separators'
        reservationsSeparatorPic = reservationsSeparator.generateLabel(pageNewReservation)
        reservationsSeparatorPic.place(x=reservationsSeparator.location[0],y=reservationsSeparator.location[1])
        reservesElement = Picture(['reservationsElement','png',120,400,1230,180],180)
        reservesElement.purpose = 'Words'
        reservesElementPic = reservesElement.generateLabel(pageNewReservation)
        reservesElementPic.place(x=reservesElement.location[0],y=reservesElement.location[1])

    def showPageOfListStudentsToEdit(self, controller, numberOfPage):
        for eachButton in self.pageNumberOfStudentsList:
            eachButton['bg']='white'
        self.pageOfListStudent = numberOfPage-1
        self.pageNumberOfStudentsList[self.pageOfListStudent]['bg'] = 'yellow'
        controller.fillStudentsList(self.studentsList, self.pageOfListStudent)

    def configureStudentsListButtons(self, pageNewReservation, controller):
        self.pageNumberOfStudentsList.append(Button(pageNewReservation, text="1", relief = SUNKEN, fg=self.colorORT, bg = 'yellow', font=self.buttonFont, compound=CENTER, height = 2, width = 4, command=lambda:self.showPageOfListStudentsToEdit(controller, 1)))
        self.pageNumberOfStudentsList[0].place(x=460, y=495)
        self.pageNumberOfStudentsList.append(Button(pageNewReservation, text="2", relief = SUNKEN, fg=self.colorORT, bg = 'white', font=self.buttonFont, compound=CENTER, height = 2, width = 4, command=lambda:self.showPageOfListStudentsToEdit(controller, 2)))
        self.pageNumberOfStudentsList[1].place(x=520, y=495)
        self.pageNumberOfStudentsList.append(Button(pageNewReservation, text="3", relief = SUNKEN, fg=self.colorORT, bg = 'white', font=self.buttonFont, compound=CENTER, height = 2, width = 4, command=lambda:self.showPageOfListStudentsToEdit(controller, 3)))
        self.pageNumberOfStudentsList[2].place(x=580, y=495)

    def fillPageNewReservationUserToReserve(self, pageNewReservation, controller):

        def onselectStudent(evt):
            listOfStudents = evt.widget
            if(self.selfReservation.get()==1):
                self.lastIndexSelectedListStudent = int(listOfStudents.curselection()[0])
                listOfStudents.selection_clear(self.lastIndexSelectedListStudent)

        def checkSelfReservation():
            if(self.selfReservation.get()==1):
                self.studentsList.selection_clear(self.studentsList.index(ACTIVE))

        self.studentsList = Listbox(pageNewReservation, width=83, height=30, font=self.listElementFont)#, xscrollcommand=self.xScroll.set,yscrollcommand=self.yScroll.set)#, sticky=N+S+E+W)
        self.studentsList.place(x=140,y=10)
        controller.fillStudentsList(self.studentsList, self.pageOfListStudents)
        self.selfReservationOption = Checkbutton(pageNewReservation, text=controller.currentLanguage.userPageContent[59], font=self.inputFont, fg='dark grey', bg=self.colorORT, variable=self.selfReservation, command=checkSelfReservation)
        self.selfReservationOption.place(x=150, y=490)
        self.studentsList.bind('<<ListboxSelect>>', onselectStudent)
        self.configureStudentsListButtons(pageNewReservation, controller)

    def fillPageNewReservationEntries(self, pageNewReservation, controller):
        self.reservationData.append(Spinbox(pageNewReservation, from_=1, to=31, font=self.inputFont, width=3, justify='center'))
        self.reservationData[0].place(x=980, y=30)
        self.reservationData.append(Spinbox(pageNewReservation, values=controller.currentLanguage.adminPageContent[44], font=self.inputFont, width=8, justify='center'))
        self.reservationData[1].place(x=1035, y=30)
        self.reservationData.append(Spinbox(pageNewReservation, from_=2018, to=2040,  font=self.inputFont, width=5, justify='center'))
        self.reservationData[2].place(x=1145, y=30)
        self.reservationData.append(Spinbox(pageNewReservation, from_=1, to=31, font=self.inputFont, width=3, justify='center'))
        self.reservationData[3].place(x=980, y=120)
        self.reservationData.append(Spinbox(pageNewReservation, values=controller.currentLanguage.adminPageContent[44], font=self.inputFont, width=8, justify='center'))
        self.reservationData[4].place(x=1035, y=120)
        self.reservationData.append(Spinbox(pageNewReservation, from_=2018, to=2040,  font=self.inputFont, width=5, justify='center'))
        self.reservationData[5].place(x=1145, y=120)
        self.reservationData.append(Text(pageNewReservation, font=self.inputFont, fg = self.colorORT, bg='white', height=6, width=38))
        self.reservationData[6].place(x=800, y=240)

    def fillPageNewReservationIndicators(self, pageNewReservation):
        reservationDayBeginningIndicator = Label(pageNewReservation, text="DD  /", font="Times 14 bold", fg = 'white', bg = self.colorORT)
        reservationDayBeginningIndicator.place(x=990, y=60)
        reservationMonthBeginningIndicator = Label(pageNewReservation, text="MM  /", font="Times 14 bold", fg = 'white', bg = self.colorORT)
        reservationMonthBeginningIndicator.place(x=1060, y=60)
        reservationYearBeginningIndicator = Label(pageNewReservation, text="YYYY", font="Times 14 bold", fg = 'white', bg = self.colorORT)
        reservationYearBeginningIndicator.place(x=1170, y=60)
        reservationDayEndingIndicator = Label(pageNewReservation, text="DD  /", font="Times 14 bold", fg = 'white', bg = self.colorORT)
        reservationDayEndingIndicator.place(x=990, y=150)
        reservationMonthEndingIndicator = Label(pageNewReservation, text="MM  /", font="Times 14 bold", fg = 'white', bg = self.colorORT)
        reservationMonthEndingIndicator.place(x=1060, y=150)
        reservationYearEndingIndicator = Label(pageNewReservation, text="YYYY", font="Times 14 bold", fg = 'white', bg = self.colorORT)
        reservationYearEndingIndicator.place(x=1130, y=150)

    def getNumberOfMonth(self, monthName):
        try:
            indexOfMonth = self.monthsName.index(monthName) + 1
            return indexOfMonth
        except ValueError:
            return -1

    def checkUserToReserve(self, controller):
        userToReserve = User(["","","","","","","","",""])
        if(self.selfReservation.get()==1):
            userToReserve = currentUser
        elif(len(self.studentsList.curselection())>0):
            userToReserve = controller.application.listStudents.students[self.studentsList.curselection()[0]]
        return userToReserve

    def isNumberBetween(self, borderNumber, numberToCheck):
        return numberToCheck>=borderNumber[0] and numberToCheck<=borderNumber[1]

    def isRegistrationInputCorrect(self, userToReserve, dataToCheck):
         userCorrect = userToReserve.usernumber!=""
         monthCorrect = self.getNumberOfMonth(dataToCheck[1].get()) >= 0
         dayCorrect = self.isNumberBetween([1,31],int(dataToCheck[0].get())) and self.isNumberBetween([2018,2040],int(dataToCheck[2].get()))
         return userCorrect and monthCorrect and dayCorrect

    def checkReservationDisponibility(self, controller, reservationToCheck):
        if (controller.application.listReservations.isReservationPeriodAvailable(reservationToCheck)):
            reservationToCheck.setAditionalInformation([self.notificationNow.get(), self.notificationNear.get()], self.reservationData[6].get(1.0,END))
            controller.application.listReservations.addReservation(reservationToCheck)
            self.updateReservationDisplay(controller)
            if(reservationToCheck.notifyNow):
                reservationAddedBodyMail = "\n\n"+controller.currentLanguage.userPageContent[171]+reservationToCheck.userToReserve.getCompleteNameSurname()+",\n\n    "+controller.currentLanguage.userPageContent[172]+controller.application.listProfessors.professor[controller.application.systemCurrentStatus.userLogged]+controller.currentLanguage.userPageContent[173]+reservationToCheck.datesOfReservation[0].strftime("%d/%B/%Y")+" to "+reservationToCheck.datesOfReservation[len(reservationToCheck.datesOfReservation)-1].strftime("%d/%B/%Y")+controller.currentLanguage.userPageContent[174]
                if(reservationToCheck.precaution!=controller.currentLanguage.userPageContent[175]):
                    reservationAddedBodyMail = reservationAddedBodyMail + "\n\n"+controller.currentLanguage.userPageContent[176]+reservationToCheck.precaution
                if(reservationToCheck.notifyNear):
                    reservationAddedBodyMail = reservationAddedBodyMail + "\n\n"+controller.currentLanguage.userPageContent[177]
                reservationAddedBodyMail = reservationAddedBodyMail + "\n\n"+controller.currentLanguage.userPageContent[178]
                controller.application.sendEmail([reservationToCheck.userToReserve.email], [controller.currentLanguage.userPageContent[179], 'Images/MailContent/newReservationMail.png', reservationAddedBodyMail, "IMAGE"])
            messagebox.showinfo(controller.currentLanguage.userPageContent[60], controller.currentLanguage.userPageContent[61])
        else:
            messagebox.showerror(controller.currentLanguage.userPageContent[62], controller.currentLanguage.userPageContent[63])

    def showRegistrationMistakes(self, controller):
        if(self.isReservationRight==None):
            messagebox.showwarning(controller.currentLanguage.userPageContent[64], controller.currentLanguage.userPageContent[65])
        elif(not self.isReservationRight[0]):
            messagebox.showwarning(controller.currentLanguage.userPageContent[66], controller.currentLanguage.userPageContent[67])
        elif(not self.isReservationRight[1]):
            messagebox.showwarning(controller.currentLanguage.userPageContent[68], controller.currentLanguage.userPageContent[69])
        elif(not self.isReservationRight[2]):
            messagebox.showwarning(controller.currentLanguage.userPageContent[70], controller.currentLanguage.userPageContent[71])
        elif(not self.isReservationRight[3]):
            messagebox.showwarning(controller.currentLanguage.userPageContent[72], controller.currentLanguage.userPageContent[73])

    def checkReservationConditions(self, controller, reservationToCheck):
        if( self.isReservationRight == [True, True, True, True] ):
            self.checkReservationDisponibility(controller, reservationToCheck)
        else:
            self.showRegistrationMistakes(controller)

    def checkAddReservation(self, controller):
        currentUser = controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged]
        userToReserve = self.checkUserToReserve(controller)
        if(self.isRegistrationInputCorrect(userToReserve, self.reservationData)):
            beginning = date(int(self.reservationData[2].get()), self.getNumberOfMonth(self.reservationData[1].get()), int(self.reservationData[0].get()))
            ending = date(int(self.reservationData[5].get()), self.getNumberOfMonth(self.reservationData[4].get()), int(self.reservationData[3].get()))
            reservationToCheck = Reservation([currentUser, userToReserve],[beginning, ending]) #[beginning.strftime("%d/%B/%Y"), ending.strftime("%d/%B/%Y")])
            self.isReservationRight = reservationToCheck.isReservationRight()
            self.checkReservationConditions(controller, reservationToCheck)
            #print("BEGINNING: ", beginning.strftime("%d/%B/%Y"))
            #print("ENDING: ", ending.strftime("%d/%B/%Y"))
        else:
            messagebox.showerror(controller.currentLanguage.userPageContent[64], controller.currentLanguage.userPageContent[74])

    def fillPageNewReservation(self, pageNewReservation, controller):
        self.fillPageNewReservationTitles(pageNewReservation, controller)
        self.fillPageNewReservationPictures(pageNewReservation, controller)
        self.fillPageNewReservationUserToReserve(pageNewReservation, controller)
        self.fillPageNewReservationEntries(pageNewReservation, controller)
        self.fillPageNewReservationIndicators(pageNewReservation)
        self.addReservationOption = Button(pageNewReservation, text=controller.currentLanguage.userPageContent[75], command=lambda: self.checkAddReservation(controller), relief = SUNKEN, fg='white', bg = 'orange', font=self.buttonFont, compound=CENTER, height = 3, width = 25)
        self.addReservationOption.place(x=880, y=460)
    ...
    def verifyContentExistance(self, controller):
        ...
        if(len(self.fermentationsList.curselection())>0):
            ...
            print("DATAOFFERMENTATIONSELECTED: ", dataOfFermentationSelected)
            #dataOfFermentationSelected = ["20181128_041550_VEL.txt", "20181128_041515_TEM.txt", "20181128_041504_POT.txt"]
    ...
    def getDataForFile(self, magnitudeName, storageFile):
        ...
        for eachFile in storageFile:
            currentFile = str(magnitudeFile)+str(eachFile)+str(magnitudeFile[0:3]).upper()+".txt"
            if(Path(currentFile).exists()):
                nowDate = date(int(eachFile[0:4]), int(eachFile[4:6]), int(eachFile[6:8]))
                print("EACHFILE: ", "|"+eachFile)
                nowTime = time(hour=int(eachFile[9:11]),minute=int(eachFile[11:13]),second=int(eachFile[13:15]))
    ...
    def configureExcelFile(self, controller):
        ...
        #nameOfFile = ["20181128_041550_VEL.txt", "20181128_041515_TEM.txt", "20181128_041504_POT.txt"]
            ...
            if(len(self.magnitudeRowList)>1):
                ...
                for column in magnitudeColumnContent:
                    for item in range(len(column)):
                        ...
                        worksheet.col(position).width = 0x0d00 + 80*25 #80*20
    ...
    def getExcelDataOfFermentation(self, controller):
        if(self.platformInformation[1].get()!="" and len(self.fermentationsList.curselection())>0):
            self.configureExcelFile(controller)
            messageExcelToDisplay = "EXCEL "+controller.currentLanguage.userPageContent[87]+" FermentationData/"+str(self.platformInformation[1].get())+".xls"
            if(self.isSendFileSelected.get()==1):
                fermentationDataFileBodyMail = "\n\n"+controller.application.getDayTimeMomentMessage(controller.currentLanguage.adminPageContent[162])+", "+controller.currentLanguage.userPageContent[172]+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].getCompleteNameSurname()+":\n"+controller.currentLanguage.userPageContent[182]+controller.application.listFermentations.fermentations[int(listOfFermentations.curselection()[0])+self.pageOfListFermentations*30].getDescriptiveInformation()+"\n\n\n"+controller.currentLanguage.userPageContent[178]
                controller.application.sendEmail([controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].email], [controller.currentLanguage.userPageContent[180], "FermentationData/"+str(self.platformInformation[1].get())+".xls", fermentationDataFileBodyMail, "FILE"])
    ...
    def getTXTDataOfFermentation(self, controller):
        if(self.platformInformation[1].get()!="" and len(self.fermentationsList.curselection())>0):
            self.configureTXTFile(controller)
            ...
            if(self.isSendFileSelected.get()==1):
                ...
                controller.application.sendEmail([controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].email], [controller.currentLanguage.userPageContent[180], "FermentationData/"+str(self.platformInformation[1].get())+".txt", fermentationDataFileBodyMail, "FILE"])
    ...
    def fillPagePlatformButtons(self, pagePlatform, controller):
        ...
        self.sendFileSelectedOption.place(x=1180, y=490)
    ...
    def fillPageNameSurname(self, pageToFill, controller):
        userName = controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].name
        userSurname = controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].surname
        professorName = Label(pageToFill, text=userName, font=self.userInfoFont, fg = 'white', bg = self.colorORT)
        professorName.place(x=300, y=30)
        professorSurname = Label(pageToFill, text=userSurname, font=self.userInfoFont, fg = 'white', bg = self.colorORT)
        professorSurname.place(x=350, y=120)
    ...
    def fillPageProfileUserStatisticsTitles(self, pageProfile, controller):
        self.fermentationsQuantityTitle = Label(pageProfile, text=controller.currentLanguage.userPageContent[96]+" "+str(controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].fermentationsQuantity), font=self.titleFont, fg = 'white', bg = self.colorORT)
        ...
        self.scrambledTitle = Label(pageProfile, text="SCRAMBLED:  "+str(controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].gamesScore[0])+controller.currentLanguage.userPageContent[183], font=self.gameFont, fg = 'DodgerBlue3', bg = self.colorORT)
        self.scrambledTitle.place(x=200, y=350)
        self.hangmanTitle = Label(pageProfile, text="HANGMAN:  "+str(controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].gamesScore[1])+controller.currentLanguage.userPageContent[183], font=self.gameFont, fg = 'lime green', bg = self.colorORT)
        self.hangmanTitle.place(x=250, y=400)
        self.triviaTitle = Label(pageProfile, text="KNOW IT ALL:  "+str(controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].gamesScore[2])+controller.currentLanguage.userPageContent[183], font=self.gameFont, fg = 'chocolate3', bg = self.colorORT)
        self.triviaTitle.place(x=300, y=450)
        #self.dodgerTitle = Label(pageProfile, text="DODGER:  ", font=self.gameFont, fg = 'DodgerBlue3', bg = self.colorORT)
        #self.dodgerTitle.place(x=200, y=350)
        #self.memoryTitle = Label(pageProfile, text="MEMORY:  ", font=self.gameFont, fg = 'lime green', bg = self.colorORT)
        #self.memoryTitle.place(x=250, y=400)
        #self.puzzleTitle = Label(pageProfile, text="PUZZLE:  ", font=self.gameFont, fg = 'chocolate3', bg = self.colorORT)
        #self.puzzleTitle.place(x=300, y=450)
        #self.nibblesTitle = Label(pageProfile, text="NIBBLES:  ", font=self.gameFont, fg = 'indian red', bg = self.colorORT)
        #self.nibblesTitle.place(x=350, y=500)
    ...
    def fillPageProfileInformation(self, pageProfile, controller):
        usernumberInfo = Label(pageProfile, text=controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].usernumber, font=self.inputFont, fg = 'white', bg = self.colorORT)
        usernumberInfo.place(x=1000, y=38)
        emailInfo = Label(pageProfile, text=controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].email, font=self.inputFont, fg = 'white', bg = self.colorORT)
        emailInfo.place(x=930, y=128)
        birthdateInfo = Label(pageProfile, text=controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].birthDate, font=self.inputFont, fg = 'white', bg = self.colorORT)
        birthdateInfo.place(x=1000, y=218)
        telephoneInfo = Label(pageProfile, text="0"+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].telephone, font=self.inputFont, fg = 'white', bg = self.colorORT)
        telephoneInfo.place(x=990, y=308)
        addressInfo = Label(pageProfile, text=controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].address, font=self.inputFont, fg = 'white', bg = self.colorORT)
        addressInfo.place(x=970, y=398)
        idNumberInfo = Label(pageProfile, text=controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].idNumber, font=self.inputFont, fg = 'white', bg = self.colorORT)
    ...
    def arePasswordConditionsRight(self, controller):
        ...
        currentPasswordCorrect = self.passwordInput[0].get() != "" and self.passwordInput[0].get() == controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].password
        anUser = User(["","","","","","","","",""])
    ...
    def checkPassword(self, controller):
        if(self.arePasswordConditionsRight(controller)):
            controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].password = self.passwordInput[1].get()
            passwordBodyMail = "\n"+controller.currentLanguage.userPageContent[172]+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].getCompleteNameSurname()+":\n    "+controller.currentLanguage.userPageContent[184]
            controller.application.sendEmail([controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].email], [controller.currentLanguage.userPageContent[185], 'Images/MailContent/passwordMail.png', passwordBodyMail, "IMAGE"])
    ...
    def sendParticularMailOption(self, controller):
        ...
        if(userSample.isEmailRight()==1):
            if(controller.isMessageRight(self.mailInformation[1])):
                controller.application.sendEmail([self.mailInformation[0].get()], [controller.currentLanguage.userPageContent[186], 'Images/MailContent/messageFromProfessorMail.png', "\n\n"+controller.currentLanguage.userPageContent[187]+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].getCompleteNameSurname()+": "+self.mailInformation[1].get(1.0, END), "IMAGE"])
    ...
    def sendAdministratorOption(self, controller):
        if(controller.isMessageRight(self.mailInformation[1])):
            controller.application.sendEmail([controller.application.admin.email], [controller.currentLanguage.userPageContent[186], 'Images/MailContent/messageFromProfessorMail.png', "\n\n"+controller.currentLanguage.userPageContent[187]+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].getCompleteNameSurname()+": "+self.mailInformation[1].get(1.0, END), "IMAGE"])
    ...
    def sendStudentsOption(self, controller):
        if(len(controller.application.listStudents.students)>0):
            if(controller.isMessageRight(self.mailInformation[1])):
                ...
                controller.application.sendEmail([allStudentsEmails], [controller.currentLanguage.userPageContent[186], 'Images/MailContent/messageFromProfessorMail.png', "\n\n"+controller.currentLanguage.userPageContent[187]+controller.application.listProfessors.professors[controller.application.systemCurrentStatus.userLogged].getCompleteNameSurname()+": "+self.mailInformation[1].get(1.0, END), "IMAGE"])
                messagebox.showinfo(controller.currentLanguage.userPageContent[89], controller.currentLanguage.userPageContent[116])
            else:
                messagebox.showerror(controller.currentLanguage.userPageContent[44], controller.currentLanguage.userPageContent[113])
        else:
            messagebox.showwarning(controller.currentLanguage.userPageContent[19], controller.currentLanguage.adminPageContent[86])
    ...
    def fillPageContactButtons(self, pageContact, controller):
        ...
        self.sendStudentsNotesOption = Button(pageContact, text=controller.currentLanguage.userPageContent[120], command=lambda:self.sendStudentsOption(controller), relief = SUNKEN, fg='white', bg = 'green', font=self.buttonFont, compound=CENTER, height = 3, width = 22)
        self.sendStudentsNotesOption.place(x=980, y=455)
    ...
    def setTabs(self, controller):
        ...
        self.pagePorts = Frame(self.nb)
        self.nb.add(self.pagePorts, text="PORTS")
        controller.setBackgroundOfTab(self.pagePorts)
        self.fillPagePorts(self.pagePorts, controller)
        ...
        self.pageNewReservation = Frame(self.nb)
        self.nb.add(self.pageNewReservation, text=controller.currentLanguage.userPageContent[6])
        controller.setBackgroundOfTab(self.pageNewReservation)
        self.fillPageNewReservation(self.pageNewReservation, controller)
    ...
    def __init__(self, parent, controller):
        ...
        controller.settingScreensControl[2] = 3
        controller.settingScreensControl[1] = 1

        #controller.application.isStudentLogged = False
